...
다른 경로의 설정 파일을 사용할 경우 `--config <path>` 옵션을 사용합니다.
...
각 단계를 확인하면서 진행하려면 다음 스크립트를 사용합니다.
...
python src/pg_schema_sync/__main__.py --commit --fk-not-valid

```

생성된 `history/validate_fks.<target>.<timestamp>.sql` 파일을 확인한 뒤, 별도 실행하여 검증합니다.
