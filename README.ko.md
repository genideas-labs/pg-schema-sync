...
*   소스 대비 누락된 확장(extension) 자동 감지 및 설치 지원 (`--install-extensions`)
...
다른 경로의 설정 파일을 사용하려면 `--config <path>` 옵션을 사용하세요.
...
*   `--config <path>`: 기본 `config.yaml` 대신 지정한 설정 파일을 사용합니다.
...
*   `history/validate_fks.gcp_test.20250405143000.sql`: `--fk-not-valid` 사용 시 생성되는 FK 검증용 SQL 파일입니다.
